...
//add by dxzhang,20110328
#if defined(CONFIG_SPI_OCORES)
static struct resource spioc_resource[] = {
	[0] = {
		.start = SPI_BASE,
		.end   = SPI_BASE + 0x1b,
...
static struct platform_device spioc_device = {
	.name		= "spioc",
	.id		= 0,
//	.id		= -1,
	.num_resources	= ARRAY_SIZE(spioc_resource),
	.resource	= spioc_resource,
	.dev		= {
	.platform_data = NULL,
	}
};

static int __init spioc_device_init(void)
{
	/* customizes platform devices, or adds new ones */
//	printk("spioc_device_init\n");
	platform_device_register(&spioc_device);
	return 0;
}
...
//add by dxzhang
	.dev_addr	= {MACADDR0,MACADDR1,MACADDR2,MACADDR3,MACADDR4,MACADDR5},
//end by dxzhang
};
...
//add by dxzhang,20110329
static struct spi_board_info orsoc_spi_board_info[] = {
...
static int __init enc28j60_device_init(void)
{
	/* customizes platform devices, or adds new ones */
	platform_device_register(&enc28j60_device);
	return 0;
}
#endif


static int __init orsoc_board_init(void)
{
	printk("orsoc_board_init\n");
//dxzhang,20110329
#if defined(CONFIG_ENC28J60) || defined(CONFIG_ENC28J60_MODULE)
	spi_register_board_info(orsoc_spi_board_info, ARRAY_SIZE(orsoc_spi_board_info));
#endif
#if defined(CONFIG_SPI_OCORES)
	spioc_device_init();
#endif
...
#endif
#if defined(CONFIG_ENC28J60) || defined(CONFIG_ENC28J60_MODULE)
	enc28j60_device_init();
#endif

}
arch_initcall(orsoc_board_init);
//end
